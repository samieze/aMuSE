...
class Tree():
    ...
    def can_be_used(self, other):
        ...
        for i in mychildren: 
            if not i in otherchildren:           
                return False                
            else:
                otherchildren.remove(i)
            ...
            if len(self.leafs()) == len(list(map(lambda x: filter_numbers(x),self.leafs()))):
                for k in othersequence.keys(): 
                    if not filter_numbers(k) in newothersequence.keys():
                        newothersequence[filter_numbers(k)] = []
                    for v in othersequence[k]:
                           newothersequence[filter_numbers(k)].append(filter_numbers(v))
                
                for i in mysequence.keys():  
                    if not filter_numbers(i) in newothersequence.keys():
                        return False
                       
                    for vs in mysequence[i]:  
                        if not filter_numbers(vs) in  newothersequence[filter_numbers(i)]:
